...
#
# Create an RK3588 GPT AOSP image for Radxa Rock 5B+.
...
UBOOT_DIR=device/radxa/rock5bplus-kernel
UBOOT_BIN="${UBOOT_DIR}/u-boot-rockchip.bin"
UBOOT_IDBLOADER="${UBOOT_DIR}/idbloader.img"
UBOOT_ITB="${UBOOT_DIR}/u-boot.itb"

BOOTLOADER_MODE=""
if [ -f "${UBOOT_BIN}" ]; then
  BOOTLOADER_MODE="combined"
elif [ -f "${UBOOT_IDBLOADER}" ] && [ -f "${UBOOT_ITB}" ]; then
  BOOTLOADER_MODE="split"
else
  exit_with_error "Missing U-Boot artifacts. Provide either ${UBOOT_BIN} or both ${UBOOT_IDBLOADER} and ${UBOOT_ITB}."
fi

VERSION=Radxa_Rock5BPlus_aosp
DATE=$(date +%Y%m%d)
...
if [ "${BOOTLOADER_MODE}" = "combined" ]; then
  echo "Writing combined U-Boot blob to sector 64..."
  # Legacy flow: a pre-composed blob written at LBA 64.
  sudo dd if="${UBOOT_BIN}" of="${IMAGE_PATH}" seek=64 bs=512 conv=notrunc status=progress
else
  echo "Writing split U-Boot binaries (idbloader + u-boot.itb)..."
  # RK35x boot layout: idbloader at LBA 64, u-boot.itb at LBA 16384.
  sudo dd if="${UBOOT_IDBLOADER}" of="${IMAGE_PATH}" seek=64 bs=512 conv=notrunc status=progress
  sudo dd if="${UBOOT_ITB}" of="${IMAGE_PATH}" seek=16384 bs=512 conv=notrunc status=progress
fi
sync
...
echo "You should now be able to write this image to your SD/NVMe and boot Rock 5B+."
